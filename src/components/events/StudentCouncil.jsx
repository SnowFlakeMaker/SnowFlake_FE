...
export default function StudentCouncil(){
    ...
    const [isAllowed, setIsAllowed] = useState(undefined); //백엔드에서 받을 학생회 합격 여부 
    const [isClosed, setIsClosed] = useState(false);
    
    if(isClosed == true) return null;

    const postCouncil = async() => {
            try{
                const response = await apiClient.post('/event/council');
    ...
    return(
        <Container>
            {isApply === undefined &&
                <>
                    <TextContainer>
                        <Text>학생회는 과의 행사부터 학우들의 편의사항을 위한 사항까지 총괄하는 일을 해.</Text>
                        <Text>학교생활 적응에 도움이 될지도 몰라. 학생회에 지원할까?</Text>
                    </TextContainer>

                    <SelectContainer>
                        <SelectOption onClick={()=>{setIsApply(true); postCouncil();}}>지원한다. (Y)</SelectOption>
            ...
            {isAllowed === true &&
                <>
                    <TextContainer>
                        <Text>학생회에 합격했어! 앞으로 열심히 활동해보자.</Text>
                    </TextContainer>
